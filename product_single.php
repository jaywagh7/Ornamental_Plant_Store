       ...
       <?php
            while($row = $result->fetch_assoc()) {
...

                {
                    ...
                    $output .= "<p id=\"discount\">Discount: ".$row['p_discount']."% </p><p id=\"old-price\"><strike>Price:".$row['p_price']." Rs</strike></p>";
                    $discount_amount = ($row['p_discount'] * $row['p_price'])/100;
                    $cr_amount = $row['p_price'] - $discount_amount;  
                    $output .= "<p id=\"new-price\">Price:".$cr_amount."Rs </p><br><p><span id=\"quantityno\">Units in Stock: <i>".$row['p_quantity']."</i></span></p>";
                    ...
                    $output .= "<p id=\"new-price\">Price: ".$row['p_price']." Rs</p><br><p><span id=\"quantityno\">Units in Stock: <i>".$row['p_quantity']."</i></span></p>";

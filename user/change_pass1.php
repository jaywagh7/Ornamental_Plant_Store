  ...
  <body>
    ...
    if ($_SERVER["REQUEST_METHOD"] == "POST") {
      ...
      if (empty($_POST["oldpass"])) {
        ...
        $check = mysqli_query($conn, " SELECT * FROM `user` WHERE `password` = '$oldpass' && `u_id` = '$id' ");
        ...
        if ($checkoldpass) {
          ...
          if (empty($_POST["newpass"])) {
            ...
            $done = mysqli_query($conn, " UPDATE `user` SET `password`='$newpass' WHERE `u_id` = '$id' ");
            ...
            if ($done) {
              ...
              $newpass_Err = "New Password Submission Failed !";
            }
